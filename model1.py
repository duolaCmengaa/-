...
class Model:
    def __init__(self, hidden_layers=(128, 32), activation=('relu', 'relu', 'softmax'), input_size=784,
        ...
        for i in range(3):
            if i == 0:
                self.layers.append(function1.Linear(input_size, hidden_layers[i]))
            else:
                if i == 1:
                    self.layers.append(function1.Linear(hidden_layers[i - 1], hidden_layers[i]))
                else:
                    self.layers.append(function1.Linear(hidden_layers[i - 1], output_size))

            self.layers.append(function1.Activation(activation[i]))

        self.lambda1 = lambda1
        self.loss = function1.SoftmaxCrossEntropyLossWithL2(self.lambda1)

        #  存储全连接层的输出值。fc层后结果(x1,x2,x3)   x0->x1->a1->x2->a2->x3->a3
        self.wx = []  
        # 存储激活值。激活函数层后结果, 第一个是输入(x0,a1,a2)
        self.ax = []  
    ...
    def update(self, x, y, learningrate):
        ...
        loss = self.loss.forward(self.wx[2], y, [1, 1, 1])
    ...
    def test(self, X, Y):
        ...
        return function1.accuracy(pred, Y)
    ...
    def train(self, X, Y, X_test, Y_test, learningrate=0.05, decay_number=0.999, batch_size=32, epochs=40, start_up=200, min_lr=1e-4,
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(X), batch_size):
                ...
                if i // batch_size % 10 == 0:
                    learningrate = function1.lr_decay(learningrate, epoch * len(X) + i, decay_number=decay_number, start_up=start_up, min_lr=min_lr)
                    if save:
